...
import { Artwork, CATEGORIES } from '../types';
      ...
      <div className="relative bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 border border-slate-700/50 rounded-2xl w-full max-w-2xl max-h-[90vh] shadow-2xl overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleUpload} className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              <label className="flex flex-col items-center justify-center h-full cursor-pointer hover:bg-slate-900/50 transition-colors group">
                ...
                <div className="flex gap-2">
                  {['PNG', 'JPG', 'JPEG', 'WEBP'].map((format) => (
                    <span 
                      key={format} 
                      className="text-[7px] font-black text-slate-600 border border-slate-800 px-1.5 py-0.5 rounded-[4px] tracking-widest group-hover:text-slate-500 group-hover:border-slate-700 transition-colors"
                    >
                      {format}
                    </span>
                  ))}
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Type (Theme)</label>
              <select 
                value={formData.type} 
                ...
                required
              >
                <option value="" disabled className="bg-slate-900">Choose a type</option>
          ...
          <div className="flex gap-4 pt-4 shrink-0">
            <button 
              ...
              onClick={onClose} 
              disabled={loading}
              className="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white py-4 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all cursor-pointer disabled:opacity-50"
            >
              Cancel
            </button>
            <button 
              type="submit" 
              disabled={loading} 
              className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-4 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all disabled:opacity-50 shadow-lg shadow-blue-500/20 cursor-pointer"
            >
